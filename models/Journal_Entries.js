...
const Get_Journal_Entrie_Info = async (je_id) => {
  try {
    ...
    var sqlQuery = "SELECT * FROM journal_entries_accounts WHERE jea_journal_entrie=?";
...
module.exports = {
  Get_Journal_Entrie_By_User: async (req, res) => {
    try {
      const { je_user } = req.body; // get data from the request
      var data = {}; // create an empty object
      ...
      //     data.journal_entrie = Get_Journal_Entrie_Info(
      //       journal_entries[i].je_id
      //     );
      //   }
      // }
  ...
  Create_Journal_Entrie: async (req, res) => {
    try {
      ...
      const je_id = common_functiions.Generate_Id()
  ...
  Update_Journal_Entrie: async (req, res) => {
    try {
      const {
        je_id,
        je_date,
